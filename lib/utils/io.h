...
#define CIPL_PERROR_CURSOR(__FMT__, __LINE__, __CURSOR__, ...)                \
  {                                                                           \
    ...
    if (__STRLEN__) {                                                         \
      CIPL_PRINTF("%5d |%5s%.*s" BRED "%.1s" RESET "%s\n", cursor.line, "",   \
                  cursor.col - 1, __LINE__, __LINE__ + cursor.col - 1,        \
                  __LINE__ + cursor.col);                                     \
      CIPL_PRINTF("%5s |%5s%*s" BRED "%c" RESET "\n", "", "", cursor.col - 1, \
                  "", '^');                                                   \
    }                                                                         \
    cursor = __B4__;                                                          \
  }
...
#define CIPL_PERROR_CURSOR_RANGE(__FMT__, __LINE__, __B__, __E__, ...)       \
  {                                                                          \
    CIPL_PRINTF("%s:%d:%d: ", filename, __B__.line, __B__.col);              \
    CIPL_PRINTF_COLOR(BRED, "error: ");                                      \
    CIPL_PRINTF(__FMT__, ##__VA_ARGS__);                                     \
    unsigned int __STRLEN__ = strlen(__LINE__);                              \
    if (__STRLEN__) {                                                        \
      CIPL_PRINTF("%5d |%5s%.*s" BRED "%.*s" RESET "%s\n", __B__.line, "",   \
